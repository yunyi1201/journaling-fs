...
void V6Replay::apply(const LogPatch &e) {
  ...
  auto block = fs_.bread(e.blockno);
  for (size_t idx = 0; idx < e.bytes.size(); idx++) {
    auto in_block_idx = idx + e.offset_in_block;
    block->at<uint8_t>(in_block_idx) = e.bytes.at(idx);
  }
  block->bdwrite();
}
...
void V6Replay::apply(const LogBlockAlloc &e) {
  if (e.zero_on_replay) {
    auto block = fs_.bget(e.blockno);
    memset(block->mem_, 0, SECTOR_SIZE);
    block->bdwrite();
  }
  freemap_.at(e.blockno) = false;
}

void V6Replay::apply(const LogBlockFree &e) { freemap_.at(e.blockno) = true; }
